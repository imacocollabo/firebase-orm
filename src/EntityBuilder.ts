...
export async function buildEntity<T>(
    meta: EntityMetaData,
    data: any,
...
async function followHierarchy<T>(params: {
    ...
    switch (params.fetchMode.mode) {
        ...
        case 'array':
            const refs = params.fetchMode.references.map(
                ref => new FirestoreReference(ref, params.reference?.transaction),
            );
            results[params.setting.propertyKey] = await Promise.all(
                refs.map(ref => {

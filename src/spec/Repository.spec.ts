...
describe('Repository test', async () => {
    ...
    context('nestedCollection', () => {
        it('should perform curd for nested collection', async () => {
            ...
            const commentRepo = getRepository(ArticleComment, {
                parentIdMapper: Entity => {
                    switch (Entity) {
                        case Article:
                            return article.id;
            ...
            const likeRepo = await getRepository(ArticleCommentLike, {
                parentIdMapper: Entity => {
                    switch (Entity) {
                        case Article:
                            return article.id;
    ...
    context('Multiple connections', () => {
        ...
        it('Should perform read/write with specified db', async () => {
            ...
            const results = await Promise.all([repo1.fetchAll(), repo2.fetchAll()]);
    ...
    context('onSnapshot', () => {
        it('should sync snap shot with relations', async () => {
            ...
            const unsubscribe = getRepository(User)
                .prepareFetcher(db => {
                    return db.limit(5);
                })
                .onSnapShot(

...
export class Repository<T extends { id: string }> {
    constructor(
        ...
        private parentIdMapper?: ParentIDMapper,
        public db?: Firestore,
    ) {}
    ...
    public async save(resource: T): Promise<T> {
        ...
        if (this.transaction && documentReference) {
            ...
            const ref = new FirestoreReference(_ref, this.transaction);
            callHook(meta, resource, 'beforeSave');
    ...
    public async update(resource: T, params: QueryPartialEntity<T>): Promise<T> {
        ...
        if (this.transaction && documentReference) {
            ...
            const ref = new FirestoreReference(this.collectionReference(meta).doc(resource.id), this.transaction);
...
export function getRepository<T extends { id: string }>(
    Entity: new () => T,
    params?: { parentIdMapper: ParentIDMapper },
    db?: Firestore,
): Repository<T> {
    if (params) {
...
export class TransactionManager {
    ...
    getRepository<T extends { id: string }>(
        Entity: new () => T,
        params?: { parentIdMapper: ParentIDMapper },
        db?: Firestore,
    ): Repository<T> {

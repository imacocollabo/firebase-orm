...
export class FirebaseEntitySerializer {
    public static serializeToJSON(
        ...
        options?: { timeStampToString?: boolean },
    ) {
...
export class FirebaseEntityDeserializer {
    public static deserializeFromJSON<T extends { id: string }>(
        Entity: ClassType<T>,
        object: object,
        parentIdMapper?: ParentIDMapper,
        options?: { stringToTimeStamp?: boolean },
    ) {
        const meta = findMeta(Entity);
        if (!meta) {
            throw new Error('object is not an Entity.');
        ...
        for (const key in object) {
            ...
            if (item[referenceCluePath]) {
                ...
                if (
                    options?.stringToTimeStamp &&
                    column instanceof _ColumnSetting &&
                    column.columnType &&
                    (column.columnType as any).now
                ) {
                    const date = new Date(item);
                    instance[key] = new (column.columnType as any)(
    ...
    public static deserializeFromJSONString<T extends { id: string }>(
        Entity: ClassType<T>,
        str: string,
        parentIdMapper?: ParentIDMapper,
    ) {
